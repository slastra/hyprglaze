...
// Story threads from quill.zig. Trail points are packed two per vec4
// (x1, y1, x2, y2), indexed thread*M + k with k=0 at the head.
const int NT = 8;   // max threads   — keep in sync with quill.zig max_threads
const int M  = 24;  // points/thread — keep in sync with quill.zig trail_points
uniform vec4 iQuillPts[96];
...
void main() {
    ...
    float wash = fbm(uv * 2.2 + vec2(iQuillTime * 0.015, -iQuillTime * 0.011));
    ...
    for (int i = 0; i < 24; i++) {
        ...
        float tw = 0.8 + 0.2 * sin(iQuillTime * 9.0 + float(i) * 2.7);
    ...
    col += (hash21(fc + fract(iQuillTime) * 100.0) - 0.5) * 0.012;
